...
int alloted[64]={0};//alloted slots for each of the course
...
void* course(void *inp){
    ...
    while(reading);//delay until input is read
    ...
    int* lab_ID=((struct course *)inp)->lab_IDs;

    while(1){
        int non_existence=0;
        for(int i=0;i<num_labs;i++){
            pthread_mutex_lock(&Ta_mutex);
            int this_lab=TA_labs[lab_ID[i]].absent;
            pthread_mutex_unlock(&Ta_mutex);

            if(this_lab==1){//anyway there are no TA's to mentor no need to check
                non_existence++;
                continue;
            }
...
void *student(void *inp){
    ...
    for(int i=0;i<3;i++){
        ...
        //student is allocated a seat for pref[i]
        ...
        if(p_student>p){//finalized a course
            printf(RED"Student %d has selected course %s permanently\n"NORMAL,ID,course_name[pref[i]]);
            return NULL;
        }
        else{//moving to next preference           
